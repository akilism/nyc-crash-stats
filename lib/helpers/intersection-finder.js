...
var intersectionFinder = (function () {
    ...
    mc.connect(mongoConnection, function(err, conn) {
      ...
      if(!err) {
        ...
        stream.on('end', function() {
          db.close();
          // console.log('Finished mongo lookup: ' + ((Date.now() - start)/1000) + 'secs');
  ...
  var populateLocations = function() {
    return Q.promise(function(resolve, reject) {
      var collectionName = 'intersections';
      var mongoConnection = mongoConnection || config.mongo;
      mc.connect(mongoConnection, function(err, conn) {
        if (err) {console.log('error connecting to mongodb\n', err); reject(err); process.exit(-1); return false; }
        if(!err) {
          db = conn;
          var collection = db.collection(collectionName);
