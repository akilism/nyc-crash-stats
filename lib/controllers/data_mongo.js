...
var formatId = function(id, type) {
  if(type === 'police_precinct' && id.length === 2) {
    return '0' + id;
  ...
  mongoConnect(connectionString).then(function(db) {
    var collection = db.collection('totals'),
      query = {
        ...
        id: (type === 'zip_code') ? parseInt(identifier) : formatId(identifier, type)
      };
      console.log(query);
      var stream = collection.find(query, {}).stream();
      var totals = [];
...
var geoTag = function(feature) {
  ...
  var options = {};
  var connectionString = 'mongodb://localhost:27017/crashstats';
  var start  = Date.now();
  var set, query;
  switch(type) {
    case 'community':
      query = {'community': {$exists : false},
               'location' : {$not : {$eq : false}},
               'loc': {$geoWithin: {$geometry: feature.geometry}}};
      set = {$set: {'community': identifier+''}};
      break;
    case 'neighborhood':
      query = {'neighborhood': {$exists: false},
               'location' : {$not : {$eq : false}},
               'loc': {$geoWithin: {$geometry: feature.geometry}}};
      set = {$set: {'neighborhood': identifier.toLowerCase() }};
      break;
    default:
      set = false;
      query = false;
  ...
  return Q.Promise(function(resolve, reject) {
    mongoConnect(connectionString).then(function(db) {
      ...
      var updateCb = function(err, result) {
        ...
        console.log('geoTag:', (Date.now() - start)/1000, 'secs');
        db.close();
        resolve(true);
        return;
      };
      ...
      if(set && query) {
        collection.updateMany(query, set, {upsert: true}, updateCb);
      }
    });
  });
