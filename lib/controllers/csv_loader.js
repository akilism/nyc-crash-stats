...
var getFileContents = function(path) {
  ...
  return Q.Promise(function(resolve, reject) {
    csv.fromPath('./splitfiles/' + path, {'objectMode': true, 'headers': headers})
...
var saveCrashes = function(iter) {
  var crash = iter.next();

  var looper = function(crash) {
    saveCrashIter(crash).then(function(result) {
      if(crash.done)  {
...
var processFiles = function(files) {
  ...
  var job = function(filename) {
    var start = Date.now();
    if(!filename) { return; }
    return getFileContents(filename).then(function(crashes) {
      count -= 1;
      console.log('files left: ' + count + ' crashes in file: ' + crashes.length);
      console.log((Date.now() - start)/1000);
      crashes = [];
      return job(files.pop());
    });
  };
  return Q.fcall(job(files.pop()));
};
