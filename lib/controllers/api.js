...
var buildFeature = function (featureProperties) {
  ...
  if (featureProperties.hasOwnProperty('borough')) {
    feature = {
      'type': 'borough',
      'identifier': featureProperties.borough,
...
var getAllGeoFiles = function (res) {
  var files = ['borough', 'citycouncil', 'community', 'neighborhood', 'precinct', 'zipcode'],
    data = [],
...
var getShapeTotals = function(req, res) {
  var type = req.query.type.toLowerCase(),
    identifier = req.query.value.toLowerCase(),
    year = req.query.year,
    // key = dataRedis.getKey(type, identifier, year),
  ...
  dataMongo.fetchShapeTotal(type, identifier, year).then(function (data) {
...
var padwith = function(char, count) {
  return function(val) {
    var acc = [val];
    var i = 0;
    for(;i < count; i++) {
...
var add00 = padwith('0', 2);

var add0 = padwith('0', 1);

var getPrecinctId = function(id) {
  if(id.length == 1) {
    return add00(id);
  } else if (id.length == 2) {
    return add0(id);
...
var getSSCQuery = function(query, typeInfo) {
  switch(typeInfo.type) {
    case 'city':
      break;
    case 'borough':
      query.borough = typeInfo.id.toUpperCase();
      break;
    case 'citycouncil':
      query.city_council_district = (typeInfo.id.length >= 2) ? typeInfo.id : add0(typeInfo.id);
      break;
    case 'community':
      query.community = typeInfo.id;
      break;
    case 'neighborhood':
      query.neighborhood = typeInfo.id.toLowerCase();
      break;
    case 'precinct':
      query.police_precinct = getPrecinctId(typeInfo.id);
      break;
    case 'zipcode':
      query.zip_code = parseInt(typeInfo.id, 10);
      break;
  }
  return query;
};
...
var streamCrashes = function (req, res) {
  ...
  ssc.connect();
  var query = {'date': {$gte: crashDate}};

  dataMongo.fetchCrashStream(collection, getSSCQuery(query, req.params), {'limit': 5000, 'sort': {'date': -1}})
...
var fetchFromMongo = function () {
  ...
  var collection = 'totals'; //'test_totals';
  var data = {};
...
var fetchShapes = function(req, res) {
  // console.log(req.query);
  var type = req.query.type;
  var identifier = req.query.value.toLowerCase();
  dataFile.getShape(type, identifier).then(function(shapes) {
...
//Return the prerolled totals for a specific area.
exports.featureTotal = function (req, res) {
  getShapeTotals(req, res);
};
